...
//MyStyle().showcrru(),
        ...
        children: [
          Container(
            ...
            child: ElevatedButton(
              onPressed: () {
                print('user_id=$user_id,name=$name,password=$password,email=$email,phonenumber=$phonenumber');
  ...
  Future<void> CheckUser() async {
    String url ='http://localhost/api1/register.php?isAdd=true&user_id=$user_id&name=$name&email=$email&phonenumber=$phonenumber&password=$password';
    try {
          Response response = await Dio().get(url);
          print(response.toString() == "true");
          if (response.toString() == "false") {
            //regiterShow();
            normalDialog(context,'อีเมล์นี้ $email มีคนอื่นใช้ไปแล้วกรุณาเปลี่ยนอีเมล์ใหม่');
          } else {
